...
check(page.includes('data-lab-mode="adaptive"')&&page.includes('data-lab-mode="mock"'),'Adaptive and mock oral modes missing');
...
check((data.match(/probes:\[/g)||[]).length>=48,'Adaptive examiner probes are not populated across the full oral set');

check(js.includes('scoreAnswer(answer,q)'),'Typed-answer coverage scoring missing');
check(js.includes("coverage.score<45?2:coverage.score<75?1:0"),'Adaptive follow-up depth is not driven by response coverage');
...
check(training.includes('Adaptive mock oral')&&training.includes('/learn/checkride-lab/'),'Training hub does not expose Checkride Lab');
for(const phrase of ['Adaptive examiner conversations','mock-oral','PDF checkride'])
...
console.log('Checkride Lab smoke passed: Pro gate, adaptive examiner probes, mock oral debrief, six-track rubrics, and real PDF packet generation verified.');
