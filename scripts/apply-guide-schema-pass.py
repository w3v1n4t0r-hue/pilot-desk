...
for p in Path('guides').glob('*.html'):
    ...
    if 'property="og:image"' not in s:
        og=f'<meta property="og:title" content="{title}"><meta property="og:description" content="{desc}"><meta property="og:type" content="article"><meta property="og:url" content="{canonical}"><meta property="og:image" content="{IMG}"><meta name="twitter:card" content="summary_large_image">'
...
for path,desc in shorten.items():
    ...
    # keep static Article description in sync
    m=pat.search(s)
    if m:
        data=json.loads(m.group(1)); data['description']=desc; data['image']=IMG
        repl='<script type="application/ld+json" data-pd-guide-schema="1">'+json.dumps(data,separators=(',',':'))+'</script>'
        s=s[:m.start()]+repl+s[m.end():]
    # sync OG description if present
