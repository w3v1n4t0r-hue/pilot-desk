...
function schema(){const p=pagePath(),h=document.querySelector('h1')?.textContent?.trim(),url=SITE+p;if(document.querySelector('script[data-pd-static-calc-schema],script[data-pd-guide-schema],script[data-pd-static-schema]'))return;let data=null;if(p==='/'){data={'@context':'https://schema.org','@type':'WebSite',name:'PilotDesk',url:SITE+'/',description}}else if(p.startsWith('/calculators/')&&h){data={'@context':'https://schema.org','@type':'WebApplication',name:h,applicationCategory:'UtilitiesApplication',operatingSystem:'Any',isAccessibleForFree:true,offers:{'@type':'Offer',price:'0',priceCurrency:'USD'},url,description,publisher:{'@type':'Organization',name:'PilotDesk',url:SITE+'/'}}}else if(p.startsWith('/guides/')&&h){data={'@context':'https://schema.org','@type':'Article',headline:h,description,publisher:{'@type':'Organization',name:'PilotDesk',url:SITE+'/'},mainEntityOfPage:url}}if(data)addJsonLd(data,'data-pd-schema');if(p.startsWith('/calculators/')&&h)addJsonLd({'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'Calculators',item:SITE+'/'},{'@type':'ListItem',position:2,name:h,item:url}]},'data-pd-breadcrumbs')}
