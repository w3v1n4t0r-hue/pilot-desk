...
function bindHeader(header){
 const nav=header.querySelector(':scope > nav')||document.createElement('nav');nav.className='pd-main-nav';nav.innerHTML=navMarkup(location.pathname);if(!nav.parentNode)header.appendChild(nav);
 let menu=header.querySelector('[data-menu]');if(!menu){menu=document.createElement('button');menu.type='button';menu.className='menu-btn';menu.dataset.menu='';menu.textContent='☰';header.insertBefore(menu,nav)}menu.setAttribute('aria-label','Open navigation');menu.setAttribute('aria-expanded','false');
 ...
 menu.addEventListener('click',e=>{e.stopPropagation();const open=nav.classList.toggle('open');menu.setAttribute('aria-expanded',String(open));menu.setAttribute('aria-label',open?'Close navigation':'Open navigation')});nav.addEventListener('click',e=>{if(e.target.closest('a')){nav.classList.remove('open');menu.setAttribute('aria-expanded','false')}});
...
async function hydrateAccount(){const link=document.querySelector('[data-pd-account-link]');if(!link)return;try{const mod=await import('https://esm.sh/@supabase/supabase-js@2.57.4');const client=mod.createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false}});const {data:{session}}=await client.auth.getSession();const avatar=link.querySelector('.pd-account-avatar'),text=link.querySelector('.pd-account-text');if(session){avatar.hidden=false;avatar.textContent=initials(session.user);text.textContent='Account';link.setAttribute('aria-label','Open PilotDesk account')}else{avatar.hidden=true;text.textContent='Sign in';link.setAttribute('aria-label','Sign in to PilotDesk')}client.auth.onAuthStateChange((_event,next)=>{if(next){avatar.hidden=false;avatar.textContent=initials(next.user);text.textContent='Account'}else{avatar.hidden=true;text.textContent='Sign in'}})}catch{}
}
function apply(){ensureStyle('/assets/pilotdesk-architecture-2026.css','pdArchitecture2026');markStandaloneApp();if(location.pathname==='/'||location.pathname==='/index.html')document.body.classList.add('pd-home-2026');document.querySelectorAll('header.topbar').forEach(bindHeader);hydrateAccount()}
ensureStyle('/assets/pilotdesk-architecture-2026.css','pdArchitecture2026');markStandaloneApp();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
})();