...
function collectCalculatorOfflineManifest(){
  ...
  // Include first-party stylesheets imported by the calculator shell stylesheet.
  // HTML links only styles.css directly, while offline calculator pages also
  // need its imported layers (including hub.css) cached on first use.
  const pending=[...assets].filter(asset=>asset.endsWith('.css'));
  const scanned=new Set();
  while(pending.length){
    const asset=pending.pop();
    if(scanned.has(asset))continue;
    scanned.add(asset);
    const file=path.join(target,asset.slice(1));
