...
for(const file of ['guides/pilot-math-formulas.html','guides/aviation-math-glossary.html']){ const h=fs.readFileSync(file,'utf8'); const words=h.replace(/<[^>]+>/g,' ').split(/\s+/).filter(Boolean).length; if(words<700) fail(`${file}: pillar content too short (${words} words)`); if(!h.includes('"image":"https://www.pilot-desk.com/assets/pilot-math-reference.svg"')) fail(`${file}: representative Article image missing`); if(!h.includes('property="og:image"')) fail(`${file}: social image missing`); }
const map=new Map();
