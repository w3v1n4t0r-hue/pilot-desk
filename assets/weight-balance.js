...
function recordRecentWb(){let a=[];try{a=JSON.parse(localStorage.getItem('pd-recent')||'[]')}catch{};const path='/weight-balance.html',title='Weight & Balance Builder';a=[{title,path,usedAt:Date.now()},...a.filter(x=>x.path!==path)].slice(0,8);saveJSON('pd-recent',a)}
...
function populateProfiles(){const arr=profiles(),requested=new URLSearchParams(location.search).get('aircraft'),active=requested||localStorage.getItem('pd-aircraft-active');profileSelect.innerHTML='<option value="">Manual / no profile</option>'+arr.map(p=>`<option value="${esc(p.id)}">${esc(p.name||p.type||'Aircraft')}</option>`).join('');if(active&&arr.some(p=>p.id===active)){profileSelect.value=active;if(requested)localStorage.setItem('pd-aircraft-active',active)}}
...
function saveScenario(){const name=scenarioName.value.trim()||`W&B ${new Date().toLocaleDateString()}`;let arr=loadJSON(KEY_SCEN,[]);const state=currentState(),id=uid();arr.unshift({...state,id,name,savedAt:Date.now()});arr=arr.slice(0,30);saveJSON(KEY_SCEN,arr);scenarioName.value=name;scenarioList();toast('W&B scenario saved on this device')}
async function saveAccountCalculation(){readDOM();const d=totals('dep'),l=totals('ldg');if(!d.ok||!l.ok){toast('Complete the loading inputs before saving');return}const service=window.PilotDeskSavedCalculations;if(!service){toast('Account save is still loading');return}const outside=envelope.length>=3?{departure:!pointInPolygon(d.cg,d.W,envelope),landing:!pointInPolygon(l.cg,l.W,envelope)}:null;const saved=await service.save({toolSlug:'weight-balance-builder',title:scenarioName.value.trim()||'Weight & Balance',inputs:currentState(),result:{departureWeight:d.W,departureMoment:d.M,departureCg:d.cg,landingWeight:l.W,landingMoment:l.M,landingCg:l.cg,envelopeChecked:Boolean(outside),outsideEnvelope:outside}});if(saved.ok){toast('Weight & Balance saved to your PilotDesk account');window.pdTrack?.('Calculation Saved',{calculator:'weight-balance-builder'});return}if(saved.reason==='signin'){location.assign(saved.url);return}toast('Could not save Weight & Balance to your account')}
...
qs('#wbSaveAccount')?.addEventListener('click',saveAccountCalculation);
