...
for(const [id,value] of Object.entries({runway:180,windDir:220,windSpeed:20,gustSpeed:''})){
  nodes.set(id,{id,value:String(value),dataset:{calcInput:true},hasAttribute(name){return id==='gustSpeed'&&name==='data-calc-optional'},closest(){return{querySelector(){return{textContent:id}}}}});
}
nodes.set('safetyWarning',safetyWarning);
const crosswindMfd=fs.readFileSync('assets/crosswind-mfd.js','utf8');
if(!crosswindMfd.includes('data-calc-optional'))throw new Error('Injected gust input must be marked optional');
...
for(const [slug,key,, ,fields] of allCalcs){
  nodes.clear();
  for(const [id,,value] of fields)nodes.set(id,{id,value:String(value),dataset:{calcInput:true},hasAttribute(){return false},closest(){return{querySelector(){return{textContent:id}}}}});
  nodes.set('safetyWarning',safetyWarning);
