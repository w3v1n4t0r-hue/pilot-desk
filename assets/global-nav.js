...
function ensureUnifiedStyle(){ensureStyle('/assets/experience.css','pdExperience');ensureStyle('/assets/efb-brief.css','pdEfbBrief')}
...
function navIcon(label){const paths={Tools:'<circle cx="12" cy="12" r="8.2"/><circle cx="12" cy="12" r="2.3"/><path d="M12 2.4v3.1M12 18.5v3.1M2.4 12h3.1M18.5 12h3.1M6.2 6.2l2.1 2.1M15.7 15.7l2.1 2.1"/>',Plan:'<circle cx="12" cy="12" r="8.5"/><path d="m8 16 3-7 5-2-3 7-5 2Z"/><path d="M12 1.8v2.4M12 19.8v2.4M1.8 12h2.4M19.8 12h2.4"/>',Weather:'<path d="M3 8h10.5c3.5 0 3.5-5 0-5-1.7 0-2.7.8-3.1 2"/><path d="M3 12h15c3.8 0 3.8 5.5 0 5.5-1.9 0-3-.9-3.4-2.2"/><path d="M3 16h7"/><path d="M5 21h10"/>',Learn:'<path d="M4 4h7v16H4zM13 4h7v16h-7z"/><path d="M7 8h1M7 12h1M16 8h1M16 12h1"/><path d="M11 6h2M11 18h2"/>'};return '<svg class="pd-nav-icon" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.45" stroke-linecap="square" stroke-linejoin="miter">'+(paths[label]||paths.Tools)+'</svg>'}
function navMarkup(path){return sections.map(s=>{const slug=String(s.label||'').toLowerCase().replace(/[^a-z0-9]+/g,'-');return `<div class="pd-nav-item" data-pd-nav-item data-section="${slug}"><button class="pd-nav-button" type="button" aria-expanded="false" ${sectionCurrent(s,path)?'aria-current="page"':''}>${navIcon(s.label)}<span>${s.label}</span><i class="pd-nav-caret"></i></button><div class="pd-nav-menu" aria-label="${s.label} menu">${s.items.map(([href,title,copy])=>`<a href="${href}"><b>${title}</b><span>${copy}</span></a>`).join('')}</div></div>`}).join('')}
...
function bindHeader(header){
 ...
 actions.innerHTML=`<a class="pd-plans-link" href="/pricing.html">Plans</a><div class="pd-search-wrap"><label class="pd-site-search">${iconSearch()}<input type="search" autocomplete="off" spellcheck="false" aria-label="Search PilotDesk" placeholder="Crosswind, VMC, IFR alternate…"><kbd class="pd-search-key" aria-hidden="true">/</kbd></label><div class="pd-search-results" role="listbox" hidden></div></div><a class="pd-account-link" href="/account.html" data-pd-account-link><span class="pd-account-avatar" hidden>PD</span><span class="pd-account-text">Sign in</span></a>`;
...
async function hydrateAccount(){const link=document.querySelector('[data-pd-account-link]');if(!link)return;try{const mod=await import('https://esm.sh/@supabase/supabase-js@2.57.4');const client=mod.createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false}});const {data:{session}}=await client.auth.getSession();const avatar=link.querySelector('.pd-account-avatar'),text=link.querySelector('.pd-account-text');const render=s=>{hydrateHomeAccount(s);window.PilotDeskSession=s||null;document.documentElement.dataset.pdAuth=s?'signed-in':'signed-out';document.dispatchEvent(new CustomEvent('pilotdesk:auth-state',{detail:{signedIn:Boolean(s),userId:s?.user?.id||null}}));if(s){avatar.hidden=false;avatar.textContent=initials(s.user);text.textContent='Account';link.setAttribute('aria-label','Open PilotDesk account')}else{avatar.hidden=true;text.textContent='Sign in';link.setAttribute('aria-label','Sign in to PilotDesk')}};render(session);client.auth.onAuthStateChange((_event,next)=>render(next))}catch{window.PilotDeskSession=null;document.documentElement.dataset.pdAuth='signed-out';document.dispatchEvent(new CustomEvent('pilotdesk:auth-state',{detail:{signedIn:false,userId:null}}));hydrateHomeAccount(null)}}
