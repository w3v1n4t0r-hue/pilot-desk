...
function render(host,{kind='empty',title='',detail='',actionLabel='',actionHref='' }={}){
 ...
 host.innerHTML=`<div class="pd-state pd-state-${cls}" role="${cls==='error'?'alert':'status'}" ${cls==='loading'?'aria-busy="true"':''}><span class="pd-state-instrument" aria-hidden="true"><i></i><i></i><i></i></span><div><b>${esc(title)}</b><span>${esc(detail)}</span>${actionLabel&&actionHref?`<a class="pd-state-action" href="${esc(actionHref)}">${esc(actionLabel)} →</a>`:''}</div></div>`;
