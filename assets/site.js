...
function recordRecent(){const path=location.pathname;if(!path.startsWith('/calculators/'))return;const title=document.querySelector('h1')?.textContent?.trim();if(!title)return;let a=[];try{a=JSON.parse(localStorage.getItem('pd-recent')||'[]')}catch{};a=[{title,path,usedAt:Date.now()},...a.filter(x=>x.path!==path)].slice(0,8);localStorage.setItem('pd-recent',JSON.stringify(a))}
