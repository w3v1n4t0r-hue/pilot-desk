...
export function sharedShell(html){
 const header=`<header class="topbar" data-pd-astro-shell><a class="brand" href="/" aria-label="PilotDesk home"><span class="brandmark" aria-hidden="true"><img src="/assets/icon.svg" width="34" height="34" alt=""></span><b>PilotDesk</b></a><button aria-label="Open navigation" aria-expanded="false" class="menu-btn" data-menu type="button">☰</button><nav class="pd-main-nav" aria-label="Main navigation">${navSections.map(s=>`<div class="pd-nav-item" data-pd-nav-item><button class="pd-nav-button" type="button" aria-expanded="false">${esc(s.label)}<i class="pd-nav-caret" aria-hidden="true"></i></button><div class="pd-nav-menu">${s.items.map(([u,t,c])=>`<a href="${u}"><b>${esc(t)}</b><span>${esc(c)}</span></a>`).join('')}</div></div>`).join('')}</nav><div class="pd-header-actions"></div></header>`;
