...
function renderDashboard(d){
 ...
 const first=weak[0];text('#pdGapNextTitle','Start with '+(first.area||'your lowest subject')+'.');text('#pdGapNextCopy',`${first.mastery}% saved mastery${first.misses?` with ${first.misses} current miss${first.misses===1?'':'es'}`:''}. Open Written Prep and use Weak-area review or Missed questions.`);
