...
const safeNext=()=>{const raw=new URL(location.href).searchParams.get('next')||'';try{const u=new URL(raw,location.origin);return u.origin===location.origin&&u.pathname.startsWith('/')?u.pathname+u.search:''}catch{return ''}};
...
async function loadSupabase(){
  ...
  try{const r=await fetch(`${SUPABASE_URL}/auth/v1/settings`,{headers:{apikey:SUPABASE_PUBLISHABLE_KEY}});if(r.ok){const d=await r.json();show('#pdGoogleSignIn',d?.external?.google===true)}}catch{}
}
...
function ensureWrittenPrepCta(){
 ...
 box.innerHTML='<b>Free FAA Written Prep</b><span>PPL · Instrument · CPL · CFI · CFII · ATP. Adaptive study remembers every miss and builds your readiness score.</span><div class="pd-account-actions"><a href="/written-prep.html">Open my written prep →</a><a href="/skill-gap.html">Check my skill gaps →</a></div>';
}
