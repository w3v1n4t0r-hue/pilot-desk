...
const corePages={
  'aircraft.html':{
    heading:'Use aircraft profiles as a planning starting point',subject:'an aircraft profile',
  ...
  'airport.html':{
    heading:'Build an airport picture, not just a lookup',subject:'an airport review',
  ...
  'weather.html':{
    heading:'Use aviation weather as a sequence of questions',subject:'a weather review',
  ...
  'metar-decoder.html':{
    heading:'Decode the report, then interpret the flight picture',subject:'a METAR decode',
  ...
  'planner.html':{
    heading:'Use the planner as a flight-planning workflow',subject:'a flight-planning workflow',
  ...
  'route-planner.html':{
    heading:'Turn a route line into a usable navlog',subject:'a route and navlog',
  ...
  'poh-chart-studio.html':{
    heading:'Practice the chart-reading process, not a memorized answer',subject:'a POH chart exercise',
  ...
  'checklist-trainer.html':{
    heading:'Use checklist practice to build a reliable cockpit sequence',subject:'checklist practice',
...
for(const [file,data] of Object.entries(corePages)){
  ...
  if(!html.includes('data-pd-core-depth')){
    ...
    const block=`<section class="info-card" data-pd-core-depth="1"><h2>${esc(data.heading)}</h2><p>${esc(data.intro)}</p><h2>Checks for this workflow</h2><ul>${checkList}</ul><p>For ${esc(data.subject||'this workflow')}, verify the source, units and reference system before trusting the result. Then compare the answer with the aircraft, airport, weather or phase of flight that makes the number useful.</p><h2>Training use for this tool</h2><p>${esc(data.training)}</p><p>This ${esc(data.subject||'training')} material is supplemental. Keep the current aircraft documents, official weather, charts, ATC instructions, NOTAMs, regulations and operator procedures as the controlling sources for an actual flight.</p><h2>Continue from here</h2><p>${data.links} · <a href="/guides.html">All aviation guides</a> · <a href="/">All calculators</a></p></section>`;
    const marker='<div class="safety-strip">';
    if(html.includes(marker)) html=html.replace(marker,`${block}${marker}`);
    else html=html.replace('</main>',`${block}</main>`);
  }
