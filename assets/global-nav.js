...
function navGlyph(label){const glyphs={Plan:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="m12 4 2.2 5.8L20 12l-5.8 2.2L12 20l-2.2-5.8L4 12l5.8-2.2Z"/></svg>',Calculators:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l4 2M6.5 12h2M15.5 12h2"/></svg>',Weather:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 12h7m-5-4h11M8 16h13"/><path d="m15 6 2-2 2 2"/></svg>',Learn:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5.5h6.5A2.5 2.5 0 0 1 14 8v10.5H7.5A2.5 2.5 0 0 0 5 21V5.5Z"/><path d="M19 5.5h-2.5A2.5 2.5 0 0 0 14 8v10.5h2.5A2.5 2.5 0 0 1 19 21V5.5Z"/></svg>'};return glyphs[label]||''}
function navMarkup(path){return sections.map(s=>`<div class="pd-nav-item" data-pd-nav-item data-section="${s.label}"><button class="pd-nav-button" type="button" aria-expanded="false" ${sectionCurrent(s,path)?'aria-current="page"':''}><span class="pd-nav-glyph">${navGlyph(s.label)}</span><span>${s.label}</span><i class="pd-nav-caret"></i></button><div class="pd-nav-menu">${s.items.map(([href,title,copy])=>`<a href="${href}"><b>${title}</b><span>${copy}</span></a>`).join('')}</div></div>`).join('')}
