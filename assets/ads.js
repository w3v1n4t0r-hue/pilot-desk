document.addEventListener('DOMContentLoaded',()=>{
  ...
  const existing=[...document.scripts].find(s=>s.src.includes('pagead2.googlesyndication.com/pagead/js/adsbygoogle.js'));const afterLoad=()=>renderSlots();
  if(existing){if(window.adsbygoogle)afterLoad();else existing.addEventListener('load',afterLoad,{once:true})}else{const script=document.createElement('script');script.async=true;script.crossOrigin='anonymous';script.src=`https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${encodeURIComponent(pub)}`;script.addEventListener('load',afterLoad,{once:true});document.head.appendChild(script)}
});
(()=>{
  ...
  if(path.startsWith('/calculators/')){load('/assets/share-enhance.js','pd-share');load('/assets/calculator-ux.js','pd-calc-ux')}
