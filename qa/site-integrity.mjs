...
function localExists(url, file=''){
  ...
  const outputRoot=path.relative(root,file).split(path.sep)[0];
  if(/^assets\/[^/]+\.[0-9a-f]{12}\.(?:css|js)$/.test(p)&&['.astro-public','dist'].includes(outputRoot)){
    return fs.existsSync(path.join(root,outputRoot,p));
  }
...
for(const file of htmlFiles){
  ...
  for(const m of raw.matchAll(/\b(?:href|src)=["']([^"']+)["']/g)){
    const u=m[1];
    if(u.startsWith('/')&&!localExists(u,file))failures.push(`${file}: missing local target ${u}`);
  }
}
