...
function pdTrack(name,props={}){try{if(typeof window.va==='function')window.va('event',name,props);else{window.vaq=window.vaq||[];window.vaq.push(['event',name,props])}}catch{}}
...
hydro(){let p=num('psi'),v=8.6*Math.sqrt(p);set(0,`${fmt(v,1)} kt`);set(1,`${fmt(p,1)} psi`);set(2,'FAA minimum dynamic hydroplaning estimate')},
...
async function shareResults(){const data={title:document.title,text:resultText(),url:location.href};try{if(navigator.share)await navigator.share(data);else await copyResults();pdTrack('calculator_share',{calculator:document.body.dataset.calc||'unknown',path:location.pathname})}catch(e){if(e?.name!=='AbortError')toast('Share failed')}}
...
function recordRecent(){const path=location.pathname;if(!path.startsWith('/calculators/')&&!['/weight-balance.html','/e6b-flight-computer.html','/route-planner.html','/weather.html','/metar-decoder.html'].includes(path))return;const title=document.querySelector('h1')?.textContent?.trim();if(!title)return;let a=[];try{a=JSON.parse(localStorage.getItem('pd-recent')||'[]')}catch{};a=[{title,path,usedAt:Date.now()},...a.filter(x=>x.path!==path)].slice(0,8);localStorage.setItem('pd-recent',JSON.stringify(a))}
function renderRecent(){const el=document.getElementById('recentTools');if(!el)return;let a=[];try{a=JSON.parse(localStorage.getItem('pd-recent')||'[]')}catch{};if(!a.length){el.innerHTML='<span class="fine">No recent tools yet.</span>';return}el.innerHTML=a.map(x=>`<a class="recent-chip" href="${x.path}">${x.title}</a>`).join('')}
...
if((location.pathname.startsWith('/guides/')||location.pathname==='/guides.html')&&!document.querySelector('script[src*="/assets/learn-step7.js"]')){const s=document.createElement('script');s.src='/assets/learn-step7.js';s.defer=true;document.head.appendChild(s)}
...
function animateResultUpdates(){
  $$('[id^="out"],.result strong').forEach(el=>{
    const observer=new MutationObserver(()=>{
      if(matchMedia('(prefers-reduced-motion: reduce)').matches)return;
      el.classList.remove('pd-result-updated');
      void el.offsetWidth;
      el.classList.add('pd-result-updated');
    });
