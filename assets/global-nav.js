...
function navMarkup(path){return sections.map((s,index)=>`<div class="pd-nav-item" data-pd-nav-item><button class="pd-nav-button" type="button" aria-expanded="false" aria-haspopup="true" aria-controls="pdNavMenu${index}" ${sectionCurrent(s,path)?'aria-current="page"':''}>${s.label}<i class="pd-nav-caret" aria-hidden="true"></i></button><div class="pd-nav-menu" id="pdNavMenu${index}">${s.items.map(([href,title,copy])=>`<a href="${href}"><b>${title}</b><span>${copy}</span></a>`).join('')}</div></div>`).join('')}
...
function bindHeader(header){
 ...
 actions.innerHTML=`<div class="pd-search-wrap"><label class="pd-site-search">${iconSearch()}<input type="search" role="combobox" autocomplete="off" spellcheck="false" aria-label="Search PilotDesk" aria-controls="pdSearchResults" aria-expanded="false" placeholder="Search PilotDesk"></label><div class="pd-search-results" id="pdSearchResults" role="status" hidden></div></div><a class="pd-account-link" href="/account.html" data-pd-account-link><span class="pd-account-avatar" hidden>PD</span><span class="pd-account-text">Sign in</span></a>`;
 const search=actions.querySelector('input'),results=actions.querySelector('.pd-search-results');
 search.addEventListener('input',()=>{renderSearchResults(results,search.value);search.setAttribute('aria-expanded',String(!results.hidden))});
 search.addEventListener('keydown',e=>{if(e.key==='Escape'){search.value='';renderSearchResults(results,'');search.setAttribute('aria-expanded','false');search.blur()}if(e.key==='Enter'){const first=results.querySelector('a');if(first){e.preventDefault();location.assign(first.href)}}});
 ...
 document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeMenus(header);nav.classList.remove('open');menu.setAttribute('aria-expanded','false');results.hidden=true;search.setAttribute('aria-expanded','false')}});
 document.addEventListener('click',e=>{if(!header.contains(e.target)){closeMenus(header);nav.classList.remove('open');menu.setAttribute('aria-expanded','false');results.hidden=true;search.setAttribute('aria-expanded','false')}});
}
...
async function apply(){const data=await ensureNavigationData();sections=Array.isArray(data.sections)?data.sections:[];searchable=Array.isArray(data.searchable)?data.searchable:[];markStandaloneApp();if(location.pathname==='/'||location.pathname==='/index.html')document.body.classList.add('pd-home-2026');document.querySelectorAll('header.topbar').forEach(bindHeader);hydrateAccount()}
markStandaloneApp();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
})();
