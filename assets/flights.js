...
function render(){renderPlan();
 ...
 <div class="pd-actions pd-saved-flight-actions"><a class="pd-btn secondary" data-load="${esc(f.id)}" href="/route-planner.html?flight=${encodeURIComponent(f.id)}">Route Planner</a><button class="pd-btn secondary" type="button" data-tomorrow="${esc(f.id)}">Copy for tomorrow</button><button class="pd-btn secondary" type="button" data-dup="${esc(f.id)}">Duplicate</button><button class="pd-btn secondary" type="button" data-edit="${esc(f.id)}">Edit</button><button class="pd-btn secondary" type="button" data-del="${esc(f.id)}">Delete</button></div></article>`}).join('')
}
...
async function duplicate(f,forTomorrow=false){
 const all=flights();
 if(!(await canAddFlight(all.length))){const status=$('#flightStatus');if(status)status.innerHTML='Free includes up to 3 saved flights. <a href="'+proHref()+'">Get PilotDesk Pro</a> to duplicate or copy another flight.';window.pdTrack?.('Pro Limit Reached',{feature:'saved_flights'});return}
